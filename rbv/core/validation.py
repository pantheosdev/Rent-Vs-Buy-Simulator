...
def get_validation_warnings(cfg: dict) -> List[str]:
    ...
    ftb = _as_bool(cfg.get("first_time") or cfg.get("first_time_buyer") or cfg.get("first_time_buyer_enabled"))
    newb = _as_bool(cfg.get("new_construction") or cfg.get("new_build"))
    ...
    if _as_bool(cfg.get("hbp_enabled", False)) and not ftb:
        warnings.append(
            "RRSP Home Buyers' Plan is enabled but the scenario is not marked as first-time buyer eligible."
        )
    if _as_bool(cfg.get("fhsa_enabled", False)) and not ftb:
